...
process_one_complex() {
    ...
    # ================= 清理逻辑 =================
    echo "Cleaning up: removing everything in $target_dir except output/"
    # 删除 target_dir 里除了 output 的所有文件/目录
    find "$target_dir" -mindepth 1 ! -name "output" -exec rm -rf {} +

    echo "Cleaning up: keeping only final PKLs inside output/"
    # 删除 output 里除两个 pkl 外的其他文件/目录
    find "$output_dir" -mindepth 1 \
        ! -name "${name}_features.pkl" \
        ! -name "${name}_features_with_masif.pkl" \
        -exec rm -rf {} +
...
if [ $success_count -gt 0 ]; then
    ...
    MERGE_OUTPUT_DIR="/es01/paratera/sce0413/czn/preprocess_pkl/merged_results5/$(basename "$CSV_FILE" .csv)"
